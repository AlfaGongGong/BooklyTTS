"""Name Replacement - čist format: riječ=zamjena"""
import re, json, os
...
class NameReplacer:
    ...
    def get_rules(self, epub_name=None):
        ...
        if os.path.exists(gf):
            with open(gf, 'r') as f: rules.update(json.load(f))
        # Knjiga (override)
        if epub_name and epub_name != '_global':
            bf = self._get_profile_file(epub_name)
            if os.path.exists(bf):
                with open(bf, 'r') as f: rules.update(json.load(f))
        return rules
    
    def add_rule(self, original, replacement, epub_name=None):
        pf = self._get_profile_file(epub_name if epub_name != '_global' else None)
        rules = {}
        if os.path.exists(pf):
            with open(pf, 'r') as f: rules = json.load(f)
    ...
    def apply(self, text, epub_name=None):
        """Zamijeni cijele riječi - sortirano po dužini (duže prvo)"""
        rules = self.get_rules(epub_name)
        result = text
        for original, replacement in sorted(rules.items(), key=lambda x: -len(x[0])):
            result = re.sub(
    ...
    def list_profiles(self):
        profiles = []
        if not os.path.exists(self.profiles_dir): return profiles
        for f in os.listdir(self.profiles_dir):
            if f.endswith('.json'):
                name = f.replace('.json','')
                with open(f'{self.profiles_dir}/{f}') as fh:
                    rules = json.load(fh)
                profiles.append({
                    'name': name, 'file': f,
    ...
    def import_moonreader(self, text, epub_name=None):
        """Import iz MoonReader ili čistog formata (riječ=zamjena ili riječ->zamjena)"""
        count = 0
        for line in text.strip().split('\n'):
            line = line.strip()
            if not line or line.startswith('#'): continue
            
            # Podrži oba formata: riječ=zamjena i riječ->zamjena
            if '=' in line:
                parts = line.split('=', 1)
            elif '->' in line:
                parts = line.split('->', 1)
    ...
    def export_moonreader(self, epub_name=None):
        return '\n'.join([f"{k}={v}" for k, v in self.get_rules(epub_name).items()])
