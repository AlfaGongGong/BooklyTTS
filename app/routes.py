...
def conversion_progress(job_id):
    if job_id not in jobs:
        return jsonify({'error': 'Job ne postoji'}), 404
    ...
    def generate():
        while True:
            ...
            if (job.get('status') in ['Zavrseno'] or
                    'Greska' in job.get('status', '')):
                break
            time.sleep(1)
...
def list_audiobooks():
    ...
    result = []
    for f in sorted(files, key=lambda x: os.path.getmtime(
...
def test_voice():
    ...
    tmp = tts.stream_chapter(
        'Dobar dan, ovo je test glasa.', voice=voice, max_chars=500)
...
def preview_replacements():
    ...
    return jsonify({
        'changes': replacer.preview(text, epub or None),
        'preview': replacer.apply(text, epub or None)[:500]
    })
