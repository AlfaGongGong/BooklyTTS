...
def status():
    import shutil
    return jsonify({'tts_ready':True, 'engine':'edge-tts', 'disk_free_mb':shutil.disk_usage('.').free//(1024*1024)})
...
def conversion_progress(job_id):
    ...
    def generate():
        while True:
            ...
            if job.get('status') in ['Zavrseno'] or 'Greska' in job.get('status',''): break
            time.sleep(1)
...
def list_profiles(): return jsonify(NameReplacer().list_profiles())
