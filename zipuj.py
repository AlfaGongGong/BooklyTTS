...
def main():
    ...
    # ✨ NOVO: Koristi naziv direktorija za ime ZIP-a
    project_name = root.name
    danas = datetime.now().strftime("%d_%m_%Y")
    
    # ✨ NOVO: ZIP se snima U ROOT direktorijum projekta
    ime_zipa = root / f"{project_name}_{danas}.zip"
    ...
    # Sačuvaj tree kao txt u root-u projekta
    tree_fajl = root / "file_tree.txt"
    tree_fajl.write_text(tree_tekst + "\n", encoding="utf-8")
    print(f"✅ Tree sačuvan u: {tree_fajl}")
    ...
    print(f"📍 Lokacija: {ime_zipa}")

    # Obriši privremeni tree fajl (opciono)
    # tree_fajl.unlink()
